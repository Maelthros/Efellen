...
namespace Server.Items
{
	...
	public class Spellbook : Item, ICraftable, ISlayer
	{
		...
		public override void ResourceChanged(CraftResource resource)
		{
			...
			TextInfo cultInfo = EnglishCulture.TextInfo;
		...
		{
			return !(this is SongBook || this is BookOfBushido || this is DeathKnightSpellbook ||
					 this is ElementalSpellbook || this is HolyManSpellbook || this is JediSpellbook ||
					 this is BookOfChivalry || this is MysticSpellbook || this is NecromancerSpellbook ||
		...
		public class SetupSpellbookEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				if (added > 0)
				{
					...
					m_From.SendMessage("Added " + added.ToString() + " scrolls.");
				...
				{
					m_From.SendMessage("No valid scrolls were found.");
				}
			}
		...
		public override bool OnEquip(Mobile from)
		{
			...
			if (this is NecromancerSpellbook)
			{
			...
			if (this is DeathKnightSpellbook)
			{
				if (from.Skills[SkillName.Knightship].Base < 30)
				{
					from.SendMessage("Your need at least a natural neophyte skill in knightship to equip that!");
			...
			if (dropped is SpellScroll && dropped.Amount == 1)
			{
				...
				else
				{
					...
					if (val >= 0 && val < BookCount)
					{
						...
						if (this is SythSpellbook || this is JediSpellbook)
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			{
				if (prop > MyServerSettings.LowerMana())
					prop = MyServerSettings.LowerMana();
				list.Add(1060433, prop.ToString()); // lower mana cost ~1_val~%
			}

			if ((prop = m_AosAttributes.LowerRegCost) != 0 && MyServerSettings.LowerReg() > 0)
			{
				if (prop > MyServerSettings.LowerReg())
					prop = MyServerSettings.LowerReg();
			...
			// Add spell/ability count with proper text
			if (this is SongBook)
			{
				if (m_Count == 1)
